...
Only keep recent data, say after 1945.

Only keep data for the candidate who won the election.
...
Only keep data for close elections, say abs(win_margin) < 5, because those are the races in which random assignment is at least somewhat plausible.
...
```{r}
linear_reg() |> 
  fit(lived_after ~ sex, data = x_pred) |> 
  tidy(conf.int = TRUE)
```
...
Note that we can't include election_result because it is always "Won"!

```{r}
linear_reg() |>
  fit(lived_after ~ win_margin + election_age + party, data = x_pred) |> 
  tidy(conf.int = TRUE)
```
